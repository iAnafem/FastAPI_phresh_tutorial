...
class CleaningsRepository(BaseRepository):
    ...
    async def get_cleaning_by_id(self, *, _id: int) -> CleaningInDB or None:
        cleaning = await self.db.fetch_one(query=GET_CLEANING_BY_ID_QUERY, values={"id": _id})
    ...
    async def update_cleaning(
        ...
        cleaning = await self.get_cleaning_by_id(_id=id)
        if not cleaning:
            return None
